...
export const fetchUsers = createAsyncThunk("users/fetchUsers", 
    async ({pageNumber, pageSize, searchKeyword, sortBy }:{pageNumber: number, pageSize: number, searchKeyword : string, sortBy: string}) => {
        const response = searchKeyword.length > 0 ? await api.get(`/users?pageNumber=${pageNumber}&pageSize=${pageSize}&searchKeyword=${searchKeyword}&sortBy=${sortBy}`, {
        headers: {
            Authorization: `Bearer ${getToken()}`
        }
    })
...
export const banUnbanUser = createAsyncThunk("users/banUnbanUser", 
async (userId: string) => {
    const response = await api.put(`/users/ban_unban/${userId}`, {}, {
        headers: {
            Authorization: `Bearer ${getToken()}`
        }
    })
